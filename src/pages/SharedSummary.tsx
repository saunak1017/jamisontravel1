    ...
    <div className="rounded-2xl border border-white/10 bg-board-panel2 shadow-soft overflow-hidden">
      ...
      <div className="p-5 space-y-3">
        {summary.rows.map((row, index) => (
          <div key={`${row.passenger}:${row.flight_number}:${index}`} className="rounded-xl border border-white/10 bg-black/20 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="text-xs font-mono text-slate-400">PASSENGER</div>
                <div className="font-semibold">{row.passenger}</div>
              </div>
              <div className="text-right">
                <div className="text-xs font-mono text-slate-400">FLIGHT</div>
                <div className="font-mono font-semibold">{row.flight_number || "—"} <span className="font-sans font-normal text-slate-400">{row.airline}</span></div>
              </div>
            </div>
            <div className="mt-5 grid grid-cols-[1fr_auto_1fr] items-center gap-4">
              <div><div className="text-2xl font-mono">{row.dep_airport}</div><div className="text-sm font-mono text-slate-400">{row.dep_date} · {row.dep_time}</div></div>
              <div className="text-sky-300">→</div>
              <div className="text-right"><div className="text-2xl font-mono">{row.arr_airport}</div><div className="text-sm font-mono text-slate-400">{row.arr_date} · {row.arr_time}</div></div>
            </div>
          </div>
        ))}
      </div>
